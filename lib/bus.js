...
Bus.prototype.getInterface = function(serviceName, objectPath, interfaceName, callback) {
	...
	serviceName = args[0];
	objectPath = args[1] || serviceName; if (objectPath[0] == '.') objectPath = serviceName + objectPath
	interfaceName = args[2] || objectPath; if (interfaceName[0] == '.') interfaceName = serviceName + interfaceName

	if(objectPath.indexOf('.') > 0) objectPath = '/' + objectPath.replace(/\./g, "/");
